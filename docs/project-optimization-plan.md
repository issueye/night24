...
- 已完成：`core_client.rs` 在当前 generation 的 stdout reader 退出时标记 core 不可用、拒绝 pending RPC 并清空事件 sender；旧 generation 退出不会影响重启后的新进程。
...
- `crates/night24-server/src/core_client.rs`：agent-core stdout reader 退出时统一关闭当前 transport 状态，拒绝 pending 请求并清理事件 sender，generation 保护避免误伤重启后的新进程。

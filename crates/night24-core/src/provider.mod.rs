...
use super::model::{ContentBlock, Message, Role};
use super::tool_executor::execute_tool;
use super::security::SecurityInspector;
    ...
    for block in &msg.content {
        match block {
            ...
            ContentBlock::ToolResponse { id: _, content: resp_content, is_error: _ } => {
                // In a real implementation, we'd emit a separate tool message here
                // For Phase 1 simplicity, append to content if no tool_calls
            ...
            ContentBlock::Thinking { text } => {
                // ignore thinking blocks for OpenAI
                let _ = text;
            }
        }
    }
        ...
        Box::pin(async move {
            ...
            let stream = try_stream! {
                if let Some(msg) = last {
                    ...
                    // Check if the text starts with "tool:" prefix
                    if text.starts_with("tool:") {
                        ...
                        if tool_input.starts_with("shell:") {
                        ...
                        } else if tool_input.starts_with("read:") {
                            ...
                            yield (Some(tool_msg), ProviderUsage::default());
                        } else if tool_input.starts_with("write:") {
                            ...
                            yield (Some(tool_msg), ProviderUsage::default());
                        } else if tool_input.starts_with("list_files:") {
                            ...
                            yield (Some(tool_msg), ProviderUsage::default());
                        } else if tool_input.starts_with("datetime:") {
                            let tool_msg = Message {
                                id: Uuid::new_v4().to_string(),
                                role: Role::Assistant,
                                content: vec![ContentBlock::ToolRequest {
                            ...
                            yield (Some(tool_msg), ProviderUsage::default());
                        } else if tool_input.starts_with("calculator:") {
                            ...
                            yield (Some(tool_msg), ProviderUsage::default());
                        } else if tool_input.starts_with("http:") {
                            ...
                            yield (Some(tool_msg), ProviderUsage::default());
                        } else if tool_input.starts_with("web_search:") {
                            ...
                            yield (Some(tool_msg), ProviderUsage::default());
                        } else if tool_input.starts_with("jq:") {
                            ...
                            let tool_result = execute_tool("developer__echo", &serde_json::json!({"text": tool_input}), std::path::Path::new("."), &security).await;
                            let content = tool_result.unwrap_or_else(|e| format!("tool error: {}", e));
